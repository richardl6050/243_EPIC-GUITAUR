...
#define AV_ADDR \
  ((volatile int*)(AV_CONFIG_BASE + 8))  // offset 8: I2C register address
...
void init() {
  ...
  // path control
  av_write(AA_PATH_CONTROL,
           0b00010000);  // dacsel on, bypass on, no input leakage
  ...
  // sampling
  av_write(SAMPLING,
           0b00000000);  // 48kHz sampling rate at 12.88 MHz external clock
