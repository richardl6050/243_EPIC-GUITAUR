#include "effects.h"

#define DELAY_SIZE 2048  // 42.6 ms of info

static int circ_buffer[DELAY_SIZE] = {0};

static int count = 0;

struct Chorus {
  int write_ptr;
  int read_ptr1;
  int read_ptr2;

  int phase_1;
  int phase_2;
};

static struct Chorus c = {
    .write_ptr = 0,
    .phase_1 = 0,
    .phase_2 = 45 //plug offset here //120 degree offset
};

//parameter summary
// 
// 1. chorus rate --> how fast we update our phase accumulator LFO!!!
// 2. chorus depth --> how much we swing
// 3. base delay --> center of our swing


static const int sine_lut[256] = {
    0, // [  0] 0.0deg
    804, // [  1] 1.4deg
    1608, // [  2] 2.8deg
    2410, // [  3] 4.2deg
    3212, // [  4] 5.6deg
    4011, // [  5] 7.0deg
    4808, // [  6] 8.4deg
    5602, // [  7] 9.8deg
    6393, // [  8] 11.2deg
    7179, // [  9] 12.7deg
    7962, // [ 10] 14.1deg
    8739, // [ 11] 15.5deg
    9512, // [ 12] 16.9deg
    10278, // [ 13] 18.3deg
    11039, // [ 14] 19.7deg
    11793, // [ 15] 21.1deg
    12539, // [ 16] 22.5deg
    13279, // [ 17] 23.9deg
    14010, // [ 18] 25.3deg
    14732, // [ 19] 26.7deg
    15446, // [ 20] 28.1deg
    16151, // [ 21] 29.5deg
    16846, // [ 22] 30.9deg
    17530, // [ 23] 32.3deg
    18204, // [ 24] 33.8deg
    18868, // [ 25] 35.2deg
    19519, // [ 26] 36.6deg
    20159, // [ 27] 38.0deg
    20787, // [ 28] 39.4deg
    21403, // [ 29] 40.8deg
    22005, // [ 30] 42.2deg
    22594, // [ 31] 43.6deg
    23170, // [ 32] 45.0deg
    23731, // [ 33] 46.4deg
    24279, // [ 34] 47.8deg
    24811, // [ 35] 49.2deg
    25329, // [ 36] 50.6deg
    25832, // [ 37] 52.0deg
    26319, // [ 38] 53.4deg
    26790, // [ 39] 54.8deg
    27245, // [ 40] 56.2deg
    27683, // [ 41] 57.7deg
    28105, // [ 42] 59.1deg
    28510, // [ 43] 60.5deg
    28898, // [ 44] 61.9deg
    29268, // [ 45] 63.3deg
    29621, // [ 46] 64.7deg
    29956, // [ 47] 66.1deg
    30273, // [ 48] 67.5deg
    30571, // [ 49] 68.9deg
    30852, // [ 50] 70.3deg
    31113, // [ 51] 71.7deg
    31356, // [ 52] 73.1deg
    31580, // [ 53] 74.5deg
    31785, // [ 54] 75.9deg
    31971, // [ 55] 77.3deg
    32137, // [ 56] 78.8deg
    32285, // [ 57] 80.2deg
    32412, // [ 58] 81.6deg
    32521, // [ 59] 83.0deg
    32609, // [ 60] 84.4deg
    32678, // [ 61] 85.8deg
    32728, // [ 62] 87.2deg
    32757, // [ 63] 88.6deg
    32767, // [ 64] 90.0deg
    32757, // [ 65] 91.4deg
    32728, // [ 66] 92.8deg
    32678, // [ 67] 94.2deg
    32609, // [ 68] 95.6deg
    32521, // [ 69] 97.0deg
    32412, // [ 70] 98.4deg
    32285, // [ 71] 99.8deg
    32137, // [ 72] 101.2deg
    31971, // [ 73] 102.7deg
    31785, // [ 74] 104.1deg
    31580, // [ 75] 105.5deg
    31356, // [ 76] 106.9deg
    31113, // [ 77] 108.3deg
    30852, // [ 78] 109.7deg
    30571, // [ 79] 111.1deg
    30273, // [ 80] 112.5deg
    29956, // [ 81] 113.9deg
    29621, // [ 82] 115.3deg
    29268, // [ 83] 116.7deg
    28898, // [ 84] 118.1deg
    28510, // [ 85] 119.5deg
    28105, // [ 86] 120.9deg
    27683, // [ 87] 122.3deg
    27245, // [ 88] 123.8deg
    26790, // [ 89] 125.2deg
    26319, // [ 90] 126.6deg
    25832, // [ 91] 128.0deg
    25329, // [ 92] 129.4deg
    24811, // [ 93] 130.8deg
    24279, // [ 94] 132.2deg
    23731, // [ 95] 133.6deg
    23170, // [ 96] 135.0deg
    22594, // [ 97] 136.4deg
    22005, // [ 98] 137.8deg
    21403, // [ 99] 139.2deg
    20787, // [100] 140.6deg
    20159, // [101] 142.0deg
    19519, // [102] 143.4deg
    18868, // [103] 144.8deg
    18204, // [104] 146.2deg
    17530, // [105] 147.7deg
    16846, // [106] 149.1deg
    16151, // [107] 150.5deg
    15446, // [108] 151.9deg
    14732, // [109] 153.3deg
    14010, // [110] 154.7deg
    13279, // [111] 156.1deg
    12539, // [112] 157.5deg
    11793, // [113] 158.9deg
    11039, // [114] 160.3deg
    10278, // [115] 161.7deg
    9512, // [116] 163.1deg
    8739, // [117] 164.5deg
    7962, // [118] 165.9deg
    7179, // [119] 167.3deg
    6393, // [120] 168.8deg
    5602, // [121] 170.2deg
    4808, // [122] 171.6deg
    4011, // [123] 173.0deg
    3212, // [124] 174.4deg
    2410, // [125] 175.8deg
    1608, // [126] 177.2deg
    804, // [127] 178.6deg
    0, // [128] 180.0deg
    -804, // [129] 181.4deg
    -1608, // [130] 182.8deg
    -2410, // [131] 184.2deg
    -3212, // [132] 185.6deg
    -4011, // [133] 187.0deg
    -4808, // [134] 188.4deg
    -5602, // [135] 189.8deg
    -6393, // [136] 191.2deg
    -7179, // [137] 192.7deg
    -7962, // [138] 194.1deg
    -8739, // [139] 195.5deg
    -9512, // [140] 196.9deg
    -10278, // [141] 198.3deg
    -11039, // [142] 199.7deg
    -11793, // [143] 201.1deg
    -12539, // [144] 202.5deg
    -13279, // [145] 203.9deg
    -14010, // [146] 205.3deg
    -14732, // [147] 206.7deg
    -15446, // [148] 208.1deg
    -16151, // [149] 209.5deg
    -16846, // [150] 210.9deg
    -17530, // [151] 212.3deg
    -18204, // [152] 213.8deg
    -18868, // [153] 215.2deg
    -19519, // [154] 216.6deg
    -20159, // [155] 218.0deg
    -20787, // [156] 219.4deg
    -21403, // [157] 220.8deg
    -22005, // [158] 222.2deg
    -22594, // [159] 223.6deg
    -23170, // [160] 225.0deg
    -23731, // [161] 226.4deg
    -24279, // [162] 227.8deg
    -24811, // [163] 229.2deg
    -25329, // [164] 230.6deg
    -25832, // [165] 232.0deg
    -26319, // [166] 233.4deg
    -26790, // [167] 234.8deg
    -27245, // [168] 236.2deg
    -27683, // [169] 237.7deg
    -28105, // [170] 239.1deg
    -28510, // [171] 240.5deg
    -28898, // [172] 241.9deg
    -29268, // [173] 243.3deg
    -29621, // [174] 244.7deg
    -29956, // [175] 246.1deg
    -30273, // [176] 247.5deg
    -30571, // [177] 248.9deg
    -30852, // [178] 250.3deg
    -31113, // [179] 251.7deg
    -31356, // [180] 253.1deg
    -31580, // [181] 254.5deg
    -31785, // [182] 255.9deg
    -31971, // [183] 257.3deg
    -32137, // [184] 258.8deg
    -32285, // [185] 260.2deg
    -32412, // [186] 261.6deg
    -32521, // [187] 263.0deg
    -32609, // [188] 264.4deg
    -32678, // [189] 265.8deg
    -32728, // [190] 267.2deg
    -32757, // [191] 268.6deg
    -32767, // [192] 270.0deg
    -32757, // [193] 271.4deg
    -32728, // [194] 272.8deg
    -32678, // [195] 274.2deg
    -32609, // [196] 275.6deg
    -32521, // [197] 277.0deg
    -32412, // [198] 278.4deg
    -32285, // [199] 279.8deg
    -32137, // [200] 281.2deg
    -31971, // [201] 282.7deg
    -31785, // [202] 284.1deg
    -31580, // [203] 285.5deg
    -31356, // [204] 286.9deg
    -31113, // [205] 288.3deg
    -30852, // [206] 289.7deg
    -30571, // [207] 291.1deg
    -30273, // [208] 292.5deg
    -29956, // [209] 293.9deg
    -29621, // [210] 295.3deg
    -29268, // [211] 296.7deg
    -28898, // [212] 298.1deg
    -28510, // [213] 299.5deg
    -28105, // [214] 300.9deg
    -27683, // [215] 302.3deg
    -27245, // [216] 303.8deg
    -26790, // [217] 305.2deg
    -26319, // [218] 306.6deg
    -25832, // [219] 308.0deg
    -25329, // [220] 309.4deg
    -24811, // [221] 310.8deg
    -24279, // [222] 312.2deg
    -23731, // [223] 313.6deg
    -23170, // [224] 315.0deg
    -22594, // [225] 316.4deg
    -22005, // [226] 317.8deg
    -21403, // [227] 319.2deg
    -20787, // [228] 320.6deg
    -20159, // [229] 322.0deg
    -19519, // [230] 323.4deg
    -18868, // [231] 324.8deg
    -18204, // [232] 326.2deg
    -17530, // [233] 327.7deg
    -16846, // [234] 329.1deg
    -16151, // [235] 330.5deg
    -15446, // [236] 331.9deg
    -14732, // [237] 333.3deg
    -14010, // [238] 334.7deg
    -13279, // [239] 336.1deg
    -12539, // [240] 337.5deg
    -11793, // [241] 338.9deg
    -11039, // [242] 340.3deg
    -10278, // [243] 341.7deg
    -9512, // [244] 343.1deg
    -8739, // [245] 344.5deg
    -7962, // [246] 345.9deg
    -7179, // [247] 347.3deg
    -6393, // [248] 348.8deg
    -5602, // [249] 350.2deg
    -4808, // [250] 351.6deg
    -4011, // [251] 353.0deg
    -3212, // [252] 354.4deg
    -2410, // [253] 355.8deg
    -1608, // [254] 357.2deg
    -804 // [255] 358.6deg
};

static const int chorus_rate[10] = {
    //ranging from 1 to 10hz
    //formula is nSamples = sampling rate / desired
    // 187, 94, 63, 47, 38, 31, 27, 23, 21, 19
    1875, 625, 375, 312, 268, 235, 188, 144, 125, 94
};

static const int chorus_depth[10] = {
    //q15 numbers
    0x0CCD,  /* [0] 0.1000 ->  3277 */
    0x199A,  /* [1] 0.2000 ->  6554 */
    0x2666,  /* [2] 0.3000 ->  9830 */
    0x3333,  /* [3] 0.4000 -> 13107 */
    0x4000,  /* [4] 0.5000 -> 16384 */
    0x4CCD,  /* [5] 0.6000 -> 19661 */
    0x599A,  /* [6] 0.7000 -> 22938 */
    0x6666,  /* [7] 0.8000 -> 26214 */
    0x7333,  /* [8] 0.9000 -> 29491 */
    0x7FFF  /* [9] 1.0000 -> 32767 */
};

const int base_delay[10] = {
    // constant for now
    48, 54, 60, 78, 90, 100, 120, 140,150, 160
};

void chorus(int *L, int *R, int effectStrength) {
  int rate = chorus_rate[effectStrength];
  int depth = chorus_depth[effectStrength];
  int delay = base_delay[effectStrength];

  // ran at every sample

  // write new data to buffer
  int dry = *L;
  circ_buffer[c.write_ptr] = dry;

  // increment phase acc every n steps
  count++;
  if (count >= rate) {
    count = 0;
    c.phase_1++;  // how many steps along LFO
    c.phase_1 = c.phase_1 & 0xFF;
    c.phase_2++;
    c.phase_2 = c.phase_2 & 0xFF;
  }

  int lfo1 = sine_lut[c.phase_1];  // returns a sine value
  int lfo2 = sine_lut[c.phase_2];

  int delay1 =
      delay + ((lfo1 * depth) >> 17);  // our lfo1 returns values up to 2^16
  int delay2 = delay + ((lfo2 * depth) >>
                        17);  // depth is our amplitude how much it swings

  c.read_ptr1 = (c.write_ptr - delay1) & (DELAY_SIZE - 1);
  c.read_ptr2 =
      (c.write_ptr - delay2) & (DELAY_SIZE - 1);  // masking ensures clamping

  int wet1 = circ_buffer[c.read_ptr1];
  int wet2 = circ_buffer[c.read_ptr2];

  // mixing
  int mix = (dry >> 1) + (wet1 >> 2) + (wet2 >> 3);

  // increment ptrs
  c.write_ptr += 1;

  c.write_ptr = (c.write_ptr & (DELAY_SIZE - 1));  // mask for cicular

  *L = mix;
  *R = mix;
}
